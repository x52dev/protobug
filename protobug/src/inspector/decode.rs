...
pub(super) fn read_input(path: Option<&Utf8Path>) -> std::result::Result<Vec<u8>, Report<Inspect>> {
    match path {
...
pub(super) fn json_input_as_text(raw_input: &[u8]) -> std::result::Result<&str, Report<Inspect>> {
    std::str::from_utf8(raw_input)
        .attach("Input format: json")
        .change_context(Inspect)
}

pub(super) fn resolve_edit_input_format(
    raw_input: &[u8],
...
pub(super) fn validate_multiple_input_format(
